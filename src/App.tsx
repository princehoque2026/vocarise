...
import { getStats, getDailyWords, saveDailyWords, updateStreak, markWordLearned, markWordWeak, getWeakWordsData, removeFromWeakWords } from './lib/storage';
import { generateDailyWords } from './services/gemini';

type Screen = 'home' | 'learning' | 'quiz' | 'progress' | 'level-select' | 'weak-review';
    ...
    if (!dailyWords) {
      try {
        ...
        if (dailyWords && dailyWords.length > 0) {
          ...
          console.error("No words generated by Gemini.");
          setWords([]);
        }
      } catch (error) {
            ...
            <>
              {screen === 'home' && (
                <HomeScreen 
                  ...
                  learnedToday={words.filter(w => stats.learnedWords.includes(w.word)).length}
                  onRetry={fetchWords}
                />
              )}
              {screen === 'level-select' && (
              ...
              {screen === 'progress' && (
                <ProgressScreen 
                  stats={stats} 
                  onStartWeakReview={() => setScreen('weak-review')} 
                />
              )}
              {screen === 'weak-review' && (
                <WeakReviewScreen 
...
function HomeScreen({ words, onStartLearning, onStartLevelQuiz, learnedToday, onRetry }: { words: Word[], onStartLearning: () => void, onStartLevelQuiz: () => void, learnedToday: number, onRetry: () => void }) {
  ...
  if (words.length === 0) {
    return (
      ...
      >
        <PrincuGuide message="Oh no! I couldn't find any words for you today. Maybe my connection is a bit fuzzy? 🐾" mood="sad" />
        <div className="card p-8 space-y-4">
          <RefreshCw size={48} className="mx-auto text-gray-300" />
          <h3 className="text-xl font-bold">No Words Found</h3>
          <p className="text-gray-500">Don't worry, we can try again! Click the button below to fetch your daily words.</p>
          <button onClick={onRetry} className="btn-primary w-full py-4 flex items-center justify-center gap-2">
            <RefreshCw size={20} /> Retry Fetching
          </button>
        </div>
      </motion.div>
    );
...
function ProgressScreen({ stats, onStartWeakReview }: { stats: UserStats, onStartWeakReview: () => void }) {
  return (
    <motion.div 
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <button 
              ...
              className="text-xs font-black bg-black text-white px-3 py-1 rounded-full hover:bg-gray-800 transition-all"
            >
              Start Review
            </button>
          </div>
...
function WeakReviewScreen({ onComplete }: { onComplete: () => void }) {
  const [words, setWords] = useState<Word[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isFlipped, setIsFlipped] = useState(false);

  useEffect(() => {
    setWords(getWeakWordsData());
  }, []);
  ...
  if (words.length === 0) {
    return (
      <div className="text-center py-20 space-y-4">
        <PrincuGuide message="You've cleared all your weak words! Amazing job! 🐱🏆" mood="excited" />
        <button onClick={onComplete} className="btn-primary">Back to Progress</button>
      </div>
    );
  }
  ...
  return (
    <motion.div 
      ...
      className="space-y-6"
    >
      <PrincuGuide message={`Reviewing your weak words. You have ${words.length} left to master! 🐾`} mood="thinking" />

      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Weak Words Review</h2>
        <button 
          onClick={handleRemove}
          className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-50 text-red-600 border border-red-100 text-xs font-bold hover:bg-red-100 transition-all"
        >
          <Trash2 size={14} /> Mark as Mastered
        </button>
      </div>
